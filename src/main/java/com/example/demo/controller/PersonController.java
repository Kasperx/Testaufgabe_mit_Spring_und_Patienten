...
public class PersonController {
    ...
    final String nameForModelData = "persons";
    final String nameForModelMessage = "message";
    final String nameForModelPermission = "showAllData";
    ...
    public void createMoreData(
        ...
        switch (isAdminAccount(username, pw)) {
            ...
            case EMPTY_PARAMETER -> {
                ...
                model.addAttribute(nameForModelMessage,
    ...
    public void findData(
            ...
            Model model,
            HttpServletResponse httpServletResponse){
        Person person = (Person) model.getAttribute(nameForModelData);
        if(person != null) {
            List<Person> personList = null;
            if (!(personList = personRepository.findByEmail(person.getEmail())).isEmpty()) {
                model.addAttribute(nameForModelData, getDataWithoutSensibleInfos(true, personList));
    ...
    //public String loadData(Model model){
        ...
        switch (isAdminAccount(username, pw)) {
            case YES -> {
                if(createDBDataOnStartup && isDatabaseEmpty()) {
                    createNewData(true);
                    model.addAttribute(nameForModelMessage,
                            "Created new data with admin account(s).");
                }
                ...
                model.addAttribute(nameForModelData, getDataWithoutSensibleInfos(true, personRepository.findAll()));
                model.addAttribute(nameForModelPermission, true);
            }
            case EMPTY_PARAMETER -> {
                log.error(IsAdmin.EMPTY_PARAMETER.toString());
                //List<Person> personList = filterPersonData(personRepository.findAll());
                //model.addAttribute("persons", personList);
                model.addAttribute(nameForModelData, getDataWithoutSensibleInfos(personRepository.findByIsAdminFalse()));
                model.addAttribute(nameForModelPermission, true);
                model.addAttribute(nameForModelMessage, IsAdmin.EMPTY_PARAMETER.toString());
            }
            case WRONG_PARAMETER -> {
                ...
                model.addAttribute(nameForModelData, getDataWithoutSensibleInfos(personRepository.findByIsAdminFalse()));
                model.addAttribute(nameForModelPermission, true);
                model.addAttribute(nameForModelMessage, IsAdmin.WRONG_PARAMETER.toString());
            }
        }
        return htmlFile;
    }
